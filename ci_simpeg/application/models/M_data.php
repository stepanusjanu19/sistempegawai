...
class M_data extends CI_Model
{
	...
	{
		$sql = "SELECT absen_lembur.id_absen,absen_lembur.kode_pegawai,absen_lembur.waktu,user.nama,absen_lembur.keterangan FROM absen_lembur
		INNER JOIN user ON absen_lembur.kode_pegawai=user.kode_pegawai";
		$query = $this->db->query($sql);
		$datas=array();
		foreach ($query->result() as $key) {
			$status = $key->keterangan;
...
// 		return $query->result_array();
	...
	function lemburtoday($tahun, $bulan, $hari)
	{
		$this->db->select('*');
		$this->db->from('absen_lembur');
		$this->db->where('keterangan', explode(" | ",'masuk')[0]);
		$this->db->where('year(waktu)', $tahun);
		$this->db->where('month(waktu)', $bulan);
		$this->db->where('day(waktu)', $hari);
		return $this->db->get();
